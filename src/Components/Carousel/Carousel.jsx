...
const Carousel = ({itemList = [], ItemTemplate, itemKey, autoplay=true, className}) => {
    ...
    const prevItem = () => {
        setActiveItemIndex(activeItemIndex => activeItemIndex === 0 ? listLength - 1 : activeItemIndex - 1);
    }
        ...
        <div className={`${styles.carousel} ${className}`}>
            <div className={styles.itemsContainer}>
                {itemList.map((item, index) =>
                    ...
                    return (<CarrouselItem item={item} key={item[itemKey] || index} isActive={isActive} ItemTemplate={ItemTemplate}/>)}
                )}
            </div>
